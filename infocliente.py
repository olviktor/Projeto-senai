...
from db import Database,Prestadores
from tkinter import messagebox
from exibepres import Vizualiza



class TelaExibirInformacoes:
    def __init__(self, user_id):
        ...
        # BOTOES
        self.botaosalva = Button(text='Salvar', font=self.fonte, command=lambda: self.salvar_dados(user_id))
        self.botaosalva.place(x=920, y=250)
        self.botaosalva.config(relief='ridge', border=4, bd=0)
        self.botaoEditar = Button(text='Editar', font=self.fonte, command=self.habilitar_edicao)
        self.botaoEditar.place(x=820, y=250)
        self.botaoEditar.config(relief='ridge', border=4, bd=0)
        self.botaoDelete=Button(text='Apagar conta',font=self.fonte,command=lambda:self.deletar(user_id))
    ...
    def salvar_dados(self, user_id):
        ...
        if nome and telefone and cpf and email and  genero and endereco and estado:
            self.db.update(user_id, nome, telefone, cpf, email, usuario, senha, genero, endereco, estado)
