...
from db import Prestadores, Database
...
class TelaExibirInformacoesProfissional:
    def __init__(self, id):
        ...
        dado = Prestadores(db=r"C:\Users\Jussara\OneDrive\Área de Trabalho\trabalho senia\prestadores.db").fetch_by_idpres(id)
        self.labelcor = Label(bg='#7ac0d7',width=200, height=200)
        self.labelcor.pack()
        print("user_id{} passado no infopres".format(id))
        print("dados valor id{}nome{}".format(dado['id'],dado['nome']))
        # textos
        ...
        self.comboprofissao.set(dado['profissao'])
        

        # BOTOES
        self.botaosalva = Button(text='Salvar', font=self.fonte, command= lambda:self.salvar_dadospres(id))
        ...
        self.botaoDelete=Button(text='Apagar conta',font=self.fonte,command=lambda:self.deletarpres(id))
    ...
    def salvar_dadospres(self, id):
        ...
        if nome and telefone and cpf and email and  genero and endereco and estado and profissao:
            self.db.updatepres(id, nome, telefone, cpf, email, usuario, senha, genero, endereco, estado, profissao)
